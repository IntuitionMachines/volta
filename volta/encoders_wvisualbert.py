...
class BertGatedSelfAttention(nn.Module):
    ...
    def forward(self, t_hidden_states, v_hidden_states, t_attention_mask, v_attention_mask):
        ...
        """
        textual_input = not isinstance(t_hidden_states, int)
        visual_input = not isinstance(v_hidden_states, int)

        if self.has_text and textual_input:
        ...
        if self.has_vision and visual_input:
        ...
        # Gated attention
        if self.has_tt and textual_input:
        ...
        if self.has_tv and textual_input and visual_input:
            # Take the dot product between "query" and "key" to get the raw attention scores.
        ...
        if self.has_vt and textual_input and visual_input:
            # Take the dot product between "query" and "key" to get the raw attention scores.
        ...
        if self.has_vv and visual_input:
            # Take the dot product between "query" and "key" to get the raw attention scores.
        ...
        if self.has_tt and self.has_tv and textual_input and visual_input:
...
class BertGatedSelfOutput(nn.Module):
    ...
    def forward(self, t_hidden_states, v_hidden_states, t_input_tensor, v_input_tensor):
        ...
        if self.single_ln and textual_input and visual_input:
...
class BertImagePredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self._freeze_weights(config)

    def _freeze_weights(self, config):
        if config.v_use_language_prototypes:
            for ix in self.decoder_dict:
                self.decoder_dict[ix].weight.requires_grad = False
                self.decoder_dict[ix].bias.requires_grad = False
...
class BertPreTrainingHeads(nn.Module):
    def __init__(self, config, bert_model_embedding_weights, add_discriminator=False):
        ...
        if add_discriminator:
            #self.discriminator = nn.Linear(config.v_hidden_size, 1)
    ...
    def forward(self, sequence_output_t, sequence_output_v, pooled_output_t, pooled_output_v,
                output_discriminator_score=False, discriminator_only=False):
        if discriminator_only:
            ...
            if self.fusion_method == "sum":
                ...
                pooled_output = self.dropout(pooled_output_t)
            elif self.fusion_method == "diff_sq":
                pooled_output = torch.norm(pooled_output_t - pooled_output_v, dim=1)
            ...
            if self.fusion_method == "diff_sq":
                seq_relationship_score = pooled_output
            else:
...
class BertModel(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.mixed_model = 'mix_' in config.image_embeddings
    ...
    def _attention_mask_processing(self, attention_mask):
    ...
    def v_forward(self, **kwargs):
        input_imgs, image_loc, image_attention_mask = kwargs['input_imgs'], \
                                                      kwargs['image_loc'], kwargs['image_attention_mask']
        if image_attention_mask is None:
            image_attention_mask = torch.ones(input_imgs.size(0), input_imgs.size(1)).type_as(torch.long)

        v_embedding_output = self.embeddings('visual', image_feat=input_imgs, image_loc=image_loc)
        extended_image_attention_mask = self._attention_mask_processing(image_attention_mask)

        return v_embedding_output, extended_image_attention_mask

    def t_forward(self, **kwargs):
        input_txt, token_type_ids, attention_mask = kwargs['input_txt'], \
        ...
        embedding_output = self.embeddings('textual', token_ids=input_txt, token_type_ids=token_type_ids)
        extended_attention_mask = self._attention_mask_processing(attention_mask)

        return embedding_output, extended_attention_mask

    def tv_forward(self, **kwargs):
        embedding_output, extended_attention_mask = self.t_forward(**kwargs)
        v_embedding_output, extended_image_attention_mask = self.v_forward(**kwargs)

        return embedding_output, extended_attention_mask, v_embedding_output, extended_image_attention_mask

    def forward(self, input_type, **kwargs):
        output_all_encoded_layers, output_all_attention_masks = kwargs['output_all_encoded_layers'], \
                                                                kwargs['output_all_attention_masks']

        if input_type == 'textual':
            embedding_output, extended_attention_mask = self.t_forward(**kwargs)
            v_embedding_output, extended_image_attention_mask = 0, 0
        elif input_type == 'visual':
            v_embedding_output, extended_image_attention_mask = self.v_forward(**kwargs)
            embedding_output, extended_attention_mask = 0, 0
        elif input_type == 'tv':
            embedding_output, extended_attention_mask, \
            v_embedding_output, extended_image_attention_mask = self.tv_forward(**kwargs)
        else:
        ...
        pooled_output_t, pooled_output_v = 0, 0
        if input_type == 'textual':
            if self.fusion_method == "vl-bert_vqa":
                text_mask = kwargs['input_txt'] != 0
...
class BertForVLPreTraining(BertPreTrainedModel):
    """Mixed-modality BERT model with multimodal pre-training heads.
    """
    def __init__(self, config):
        super(BertForVLPreTraining, self).__init__(config)

        self.bert = BertModel(config)
        add_discriminator = True if config.objective == 3 or config.objective == 4 else False
        self.cls = BertPreTrainingHeads(config, self.bert.embeddings.word_embeddings.weight, add_discriminator)
    ...
    def freeze_discriminator(self, freeze_bert=False):
        self.cls.discriminator.requires_grad_(False)
        if freeze_bert:
            for param in self.bert.encoder.parameters():
                param.requires_grad = False

    def defreeze_discriminator(self, defreeze_bert=False):
        self.cls.discriminator.requires_grad_(True)
        if defreeze_bert:
            for param in self.bert.encoder.parameters():
                param.requires_grad = True

    def tv_forward(self, input_type, **kwargs):
    ...
    def forward(self,
        ...
        # todo: add object tags as language tokens
        kwargs.update(dict())
        sequence_output_t, sequence_output_v, pooled_output_t, pooled_output_v, \
        (all_attention_mask_t, all_attention_mask_v) = self.tv_forward('tv', **kwargs)
        ...
        if confuse_discriminator_only:
            ...
            conf_discriminator_loss_v = self.loss_bce(discriminator_score_v,
                                                      torch.zeros_like(discriminator_score_v, device=device))

            return conf_discriminator_loss_t, conf_discriminator_loss_v
        else:
            # Vision loss
            img_loss = 0
            for ix, weight in self.visual_target_weights.items():
                if self.config.add_global_imgfeat == "last":
                    ...
                    prediction_scores_global_v = prediction_scores_v_dict[ix][:, 0]
            ...
            if add_multi_label_loss_t:
                ...
                multi_label_target_t[masked_lm_labels == -1] = num_classes
                multi_label_target_t = torch.nn.functional.one_hot(multi_label_target_t, num_classes=num_classes + 1)
                multi_label_target_t = torch.sum(multi_label_target_t, dim=1)[::, :-1] > 0  # (batch, num_classes)
                multi_label_target_t = multi_label_target_t.type_as(prediction_scores_t)
                multi_label_target_t = self._smooth_label(multi_label_target_t, alpha=1e-2)
